...
/* Preview gambar sekarang MENUTUPI kotak upload, bukan di bawahnya */
.foto-preview-img {
    position: absolute;
    inset: 0;
...
.foto-upload-area.has-image .foto-placeholder { display: none; }
.foto-upload-area.has-image .foto-preview-img { display: block; }
...
.foto-upload-area.has-image .foto-change-hint { display: flex; }
.foto-upload-area.has-image:hover .foto-change-hint { opacity: 1; }
...
<form method="POST" action="{{ route('rekam_medis.store', $pasien->id) }}" enctype="multipart/form-data" id="formRekamMedis">
@csrf
        ...
        <div class="card" style="padding:24px;">
            ...
            <div class="form-group">
                ...
                <input type="date" name="tanggal_tindakan" class="form-control"
                       value="{{ old('tanggal_tindakan', date('Y-m-d')) }}"
            ...
            <div class="form-row-2">
                <div class="form-group">
                    ...
                    <select name="jenis_perawatan" class="form-control">
                        ...
                        <option value="Facial Glow" {{ old('jenis_perawatan') == 'Facial Glow' ? 'selected' : '' }}>Facial Glow</option>
                        <option value="Chemical Peeling" {{ old('jenis_perawatan') == 'Chemical Peeling' ? 'selected' : '' }}>Chemical Peeling</option>
                        <option value="Laser Therapy" {{ old('jenis_perawatan') == 'Laser Therapy' ? 'selected' : '' }}>Laser Therapy</option>
                        <option value="Botox" {{ old('jenis_perawatan') == 'Botox' ? 'selected' : '' }}>Botox</option>
                        <option value="Konsultasi" {{ old('jenis_perawatan') == 'Konsultasi' ? 'selected' : '' }}>Konsultasi</option>
                        <option value="Lainnya" {{ old('jenis_perawatan') == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                    </select>
                </div>
                <div class="form-group">
                    <label class="form-label">Produk Digunakan</label>
                    <input type="text" name="catatan_tindakan" class="form-control" placeholder="Nama produk/obat..." value="{{ old('catatan_tindakan') }}">
                </div>
            </div>
                ...
                <div class="foto-grid">
                    <div>
                        <div class="foto-upload-area" onclick="this.querySelector('input').click()">
                            <input type="file" name="foto_before" accept="image/*" onchange="previewFoto(this, 'prev-before', this.closest('.foto-upload-area'))">
                            ...
                            <img id="prev-before" class="foto-preview-img" alt="Preview foto sebelum">
                            <div class="foto-change-hint">Klik untuk ganti foto</div>
                        </div>
                    </div>
                    <div>
                        <div class="foto-upload-area" onclick="this.querySelector('input').click()">
                            <input type="file" name="foto_after" accept="image/*" onchange="previewFoto(this, 'prev-after', this.closest('.foto-upload-area'))">
                            <div class="foto-placeholder">
                                <div class="foto-icon">
                                    <svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                        <rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/>
                            ...
                            <img id="prev-after" class="foto-preview-img" alt="Preview foto sesudah">
                            <div class="foto-change-hint">Klik untuk ganti foto</div>
                        </div>
                    </div>
                </div>
    ...
    <div class="right-panel">
        ...
        <div class="card" style="padding:24px;">
            ...
            <div class="form-group">
                ...
                <input type="date" name="tanggal_kontrol" class="form-control" value="{{ old('tanggal_kontrol') }}" min="{{ date('Y-m-d') }}">
            </div>
        ...
        <div class="card" style="padding:24px;">
            ...
            <p style="font-size:12px; color:#9B9B9B; margin-bottom:16px; text-transform:uppercase; letter-spacing:0.3px;">Pilih status kepulangan pasien</p>

            <label class="status-option selected" id="opt-selesai">
                <input type="radio" name="_status_kunjungan" value="selesai" checked onchange="setStatus('selesai')">
            ...
            <label class="status-option" id="opt-meninggal">
                <input type="radio" name="_status_kunjungan" value="meninggal" onchange="setStatus('meninggal')">
