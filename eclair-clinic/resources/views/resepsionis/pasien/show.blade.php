...
<div class="card" style="max-width:760px;">
    <div class="profile-banner">
        ...
        <div>
            <div class="profile-banner-name">
                {{ $pasien->nama_pasien }}
                @if(!$pasien->is_aktif)
                    <span style="font-size:11px; font-weight:600; color:#B25656; background:#FBE4DD; padding:3px 8px; border-radius:6px; margin-left:6px;">
                        NONAKTIF (DIGABUNG)
                    </span>
                @endif
            </div>
    ...
    <div class="detail-actions">
        @if($pasien->is_aktif)
            ...
            <a href="{{ route('pasien.merge.form', $pasien->id) }}" class="btn-secondary">Gabung Data Duplikat</a>
        @else
            <span class="detail-item-value">
                Data digabung ke:
                <a href="{{ route('pasien.show', $pasien->merged_ke_id) }}">
                    {{ $pasien->mergedKe->nama_pasien ?? '-' }}
                </a>
            </span>
        @endif
    </div>
